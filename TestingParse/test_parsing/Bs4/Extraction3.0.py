...
#     next_sibling = soup.p
#     while True:
#          if len(next_sibling.text.replace(' ', '')) % 2 == 0:
#             total_id_sum += int(next_sibling.get('id'))
#             total_class_sum += int(next_sibling.get('class')[0])
#
...
def sum_even_length_ids(html):
    ...
    for p in p_tags:
        text_length = len(p.text.replace(" ", ""))  # Считаем длину текста без пробелов
        if text_length % 2 == 0:
            if p.get('id'):  # Если есть атрибут ID, добавляем его значение к сумме
                total_id_sum += int(p['id'])
            if p.get('class'):  # Если есть атрибут class, добавляем все его значения к сумме
                total_class_sum += sum([int(cls) for cls in p['class']])

    return total_id_sum, total_class_sum
id_sum, class_sum = sum_even_length_ids(html)
print(f"Сумма ID и CLASS тегов <p> с чётной длиной текста без пробелов: {id_sum + class_sum}")
